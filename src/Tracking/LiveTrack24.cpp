...
LiveTrack24::UserID
LiveTrack24::GetUserID(JobRunner &runner, const TCHAR *username, const TCHAR *password)
{
                           ...
                           VehicleType vtype, const TCHAR *vname)
{
  ...
  TCHAR url[2048];
  _sntprintf(url, 2048, _T("http://%s/track.php?leolive=2&sid=%u&pid=%u&client=%s&v=%s&user=%s&pass=%s&vtype=%u&vname=%s"),
...
LiveTrack24::SendPosition(SessionID session, unsigned packet_id,
                          ...
                          BrokenDateTime time_utc)
{
  // http://www.livetrack24.com/track.php?leolive=4&sid=42664778&pid=321&
  //   lat=22.3&lon=40.2&alt=23&sog=40&cog=160&tm=1241422845

  TCHAR url[2048];
  _sntprintf(url, 2048, _T("http://%s/track.php?leolive=4&sid=%u&pid=%u&lat=%f&lon=%f&alt=%d&sog=%d&cog=%d&tm=%lld"),
             GetServer(), session, packet_id,
             (double)position.Latitude.value_degrees(),
             (double)position.Longitude.value_degrees(),
             altitude, ground_speed,
             (unsigned)track.as_bearing().value_degrees(),
             time_utc.ToUnixTimeUTC());
...
LiveTrack24::EndTracking(SessionID session, unsigned packet_id)
{
  ...
  TCHAR url[2048];
  _sntprintf(url, 2048, _T("http://%s/track.php?leolive=3&sid=%u&pid=%u&prid=0"),
             GetServer(), session, packet_id);
...
void
LiveTrack24::SetTest(bool _use_test_server)
{
...
LiveTrack24::GetServer()
{
  ...
  //return use_test_server ? _T("localhost:8080") : _T("www.livetrack24.com");
}
